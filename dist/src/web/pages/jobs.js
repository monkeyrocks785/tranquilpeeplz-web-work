...
// ---------------------------------------------------------------------------
export function jobCard(job, company) {
...
export function jobSearchPage(p) {
    ...
    const options = (list, name, label) => `<select id="f-${name}" name="${name}" class="field" aria-label="${label}">
    ...
    ${rv(`<form method="get" action="/job-search" class="mt-10 rounded-[1.75rem] border border-ink/10 bg-cream p-5 md:p-6">
...
export function jobDetailPage(p) {
    ...
    const meta = [
        ...
        .map((m) => `<div class="rounded-2xl border border-ink/10 bg-cream p-4">
        ...
        .join("");
    const list = (items, iconCls) => `<ul class="mt-4 space-y-3">${items
  ...
  <div class="mt-8 grid gap-10 lg:grid-cols-[1.5fr_1fr]">
    <div>
      ${rv(`<div class="flex flex-wrap items-center gap-2.5">
      ...
      ${job.skills.length ? rv(`<h2 class="font-display mt-9 text-2xl font-medium tracking-tight">Key skills</h2>
        ...
        .join("")}</div>`) : ""}
      ${rv(`<div class="mt-11 rounded-3xl bg-ink p-7 text-cream md:p-8">
    ...
    <aside class="lg:sticky lg:top-28 lg:self-start">
      ${rv(`<div class="rounded-[1.75rem] border border-ink/10 bg-cream p-6 shadow-xl shadow-ink/5 md:p-7">
          ${p.applied
...
export function postJobPage(p) {
    ...
    const noteHtml = notes
        .map((n, i) => rv(`<div class="flex gap-4">
    ...
    if (p.success) {
      ...
      <p class="mx-auto mt-2 max-w-md text-[14.5px] text-ink-soft">${esc(p.message ?? "")}</p>
        ...
        const industryOpts = [
            ...
            ...p.industries.map((i) => `<option value="${esc(i)}"${p.values?.industry === i ? " selected" : ""}>${esc(i)}</option>`),
