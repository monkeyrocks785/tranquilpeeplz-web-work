...
function stat(iconName, value, label, tone = "default") {
...
export function adminPage(p) {
    const pendingRows = p.pending.length
        ? p.pending
            .map(({ job, company }) => `<div class="rounded-3xl border border-gold/50 bg-cream p-5 md:p-6">
    ...
    const openRows = p.open
        .map(({ job, company, applicationCount }) => `<div class="flex flex-wrap items-center gap-4 rounded-3xl border border-ink/10 bg-cream p-5">
      <div class="min-w-0 flex-1">
    ...
    const closedRows = p.closed.length
      ...
      <div class="mt-5 space-y-3">
        ${p.closed
            .slice(0, 8)
            .map(({ job, company }) => `<div class="flex flex-wrap items-center gap-4 rounded-3xl border border-ink/10 bg-cream/60 p-5 text-ink/60">
              <div class="min-w-0 flex-1">
    ...
    const applicationRows = p.applications.length
        ...
        <table class="w-full min-w-[760px] text-left text-[13.5px]">
          ...
          <tbody>
            ${p.applications
            .map(({ application, job, company }) => `<tr class="border-b border-ink/5 last:border-0">
    ...
    const emailRows = p.emails.length
        ? p.emails
            .map((m) => `<div class="flex flex-wrap items-center gap-4 rounded-3xl border border-ink/10 bg-cream p-5">
            <div class="min-w-0 flex-1">
