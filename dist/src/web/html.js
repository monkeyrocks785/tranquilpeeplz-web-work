...
function header(path) {
    const isHome = path === "/";
    const links = NAV.map((n) => `<a href="${n.href}" class="link-underline text-[13px] font-medium tracking-wide ${path.startsWith(n.href) ? "text-accent" : (isHome ? "text-cream/90 hover:text-cream" : "text-ink/75 hover:text-ink")}">${n.label}</a>`).join("\n      ");
    const mobileLinks = [{ href: "/", label: "Home" }, ...NAV]
        .map((n, i) => `<a href="${n.href}" class="group flex items-baseline gap-4 border-b border-${isHome ? 'cream/10' : 'ink/10'} py-4">
...
function footer() {
    const quickLinks = [
        ...
        .map(([h, l]) => `<li><a href="${h}" class="text-[15px] text-cream/75 transition-colors hover:text-accent">${l}</a></li>`)
        .join("\n            ");
    const services = [
        ["/staffing", "Staffing"],
...
export function sectionHeading(p) {
    ...
    return rv(`<div class="max-w-2xl ${alignCls}">
...
export function marquee(items) {
    const row = [...items, ...items]
        .map((i) => `<span class="flex shrink-0 items-center gap-10 font-display text-2xl font-medium tracking-tight text-ink/60 italic">${esc(i)}<span class="inline-block h-2 w-2 rounded-full bg-accent/70"></span></span>`)
        .join("");
...
export function formAlert(state) {
    if (!state?.message)
        return "";
    const cls = state.ok
