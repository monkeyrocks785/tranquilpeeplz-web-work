...
import { aboutPage, contactPage, forEmployerPage, forJobSeekerPage, privacyPage, servicePage, SERVICE_PAGES, } from "./web/pages/marketing.js";
...
import { INDUSTRIES, getAdminStats, getBlogPostBySlug, getJobBySlug, getPublicStats, getSimilarJobs, listAdminJobs, listAllApplications, listBlogPosts, listEmailLogs, listFeaturedJobs, listJobLocations, listJobs, listAllApplications as listAppsRaw, } from "./lib/data.js";
...
function send(res, title, path, body, description) {
...
app.post("/jobs/:slug", asyncRoute(async (req, res, next) => {
    ...
    if (result.ok) {
        return send(res, "Application sent · Tranquil Peeplz", "/job-search", jobDetailPage({ job: row.job, company: row.company, similar, applied: true, applyMessage: result.message }));
    }
    send(res, `${row.job.title} · Tranquil Peeplz`, "/job-search", jobDetailPage({ job: row.job, company: row.company, similar, applyMessage: result.message, applyErrors: result.errors, applyValues: values }));
}));
...
app.post("/post-a-job", asyncRoute(async (req, res) => {
    ...
    if (typeof req.body.website === "string" && req.body.website.length > 0) {
        ...
        return send(res, "Post a Job · Tranquil Peeplz", "/post-a-job", postJobPage({ industries: [...INDUSTRIES], success: true, message: "Thanks — your role has been received." }));
    }
    const result = await submitJobForReview(values);
    if (result.ok) {
        return send(res, "Post a Job · Tranquil Peeplz", "/post-a-job", postJobPage({
            industries: [...INDUSTRIES],
            success: true,
            message: "Thanks! Your role is with our team for approval. Check your inbox — a confirmation email is on its way. It typically goes live within one business day.",
        }));
    }
    send(res, "Post a Job · Tranquil Peeplz", "/post-a-job", postJobPage({ industries: [...INDUSTRIES], message: result.message, errors: result.errors, values }));
}));
// ---------------------------------- Blog ------------------------------------
...
app.post("/contact-us", asyncRoute(async (req, res) => {
    const values = fieldValues(req.body);
    if (typeof req.body.website === "string" && req.body.website.length > 0) {
        return send(res, "Contact Us · Tranquil Peeplz", "/contact-us", contactPage({ success: true, message: "Thanks for reaching out!" }));
    }
    ...
    if (!parsed.success) {
        return send(res, "Contact Us · Tranquil Peeplz", "/contact-us", contactPage({ message: "Please fix the highlighted fields.", errors: parsed.error.flatten().fieldErrors, values }));
    }
    ...
    send(res, "Contact Us · Tranquil Peeplz", "/contact-us", contactPage({ success: true, message: "Thanks for reaching out — our team will get back within one business day." }));
}));
// ---------------------------------- Admin -----------------------------------
...
app.post("/admin/jobs/:id/:action", asyncRoute(async (req, res) => {
    ...
    const [job] = await db
        ...
        .limit(1);
    if (status) {
        await db.update(jobs).set({ status: status }).where(eq(jobs.id, id));
        // Send approval email to employer when job is approved
        if ((action === "approve" || action === "reopen") && job) {
            await sendJobApprovedEmail(job.job.id, job.job.contactEmail, job.job.contactName, job.job.title, job.companyName);
        }
    }
    res.redirect("/admin");
}));
...
app.get("/api/applications.csv", asyncRoute(async (req, res) => {
    ...
    const csv = toCsv(["Applied At", "Job Title", "Company", "Applicant Name", "Applicant Email", "Applicant Phone", "Resume URL", "Cover Note", "Status"], rows.map(({ application, job, company }) => [
