...
async function doBootstrap(): Promise<void> {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (result.seeded) {
        console.log(
          `[db] Schema ready. Seeded demo data (${result.jobs} jobs, companies, blog posts).`,
        );
      } else {
      ...
      lastError = err;
      console.error(
        `[db] Bootstrap attempt ${i + 1}/${attempts} failed:`,
  ...
  console.error(
    "[db] Bootstrap gave up. Check DATABASE_URL. The app will still serve " +
